...
@section('title', 'Feedback - Instructor Dashboard')
                        ...
                        <tr>
                            ...
                            <td>
                                @isset($ticket->reply)
                                    {{ $ticket->reply }}
                                @else
                                    <form action="{{ route('instructor.reply_to_feedback', ['id' => $ticket->id]) }}"
                                        method="post" class="flex gap-x-1.5">
                                        @csrf
                                        @method('put')
                                        <input type="text" name="reply" id="reply" value="{{ old('reply') }}"
